...
Análisis de Admisiones, Retención y Satisfacción Estudiantil
...
st.title("🎓 Panel de Control Universitario")
st.caption("Explora indicadores de admisión, matrícula, retención y satisfacción estudiantil.")
...
anios_seleccionados = st.sidebar.multiselect("Seleccionar año(s)", options=anios, default=[anios[-1]])
terminos_seleccionados = st.sidebar.multiselect("Seleccionar término(s)", options=terminos, default=terminos)
...
col3.metric("Retención promedio", f"{promedio_retencion:.2f}%")

st.markdown("---")

#Pestañas de visualización
pestana1, pestana2, pestana3 = st.tabs(["📈 Series temporales", "🏫 Facultades", "📋 Tabla de datos"])

#Pestaña 1: Series
with pestana1:
    st.subheader("Evolución de solicitudes por año y término")
    grafico_solicitudes = px.line(
        ...
        color=COL_TERMINO,
        markers=True,
        title="Tendencia de solicitudes de admisión"
    )
    st.plotly_chart(grafico_solicitudes, use_container_width=True)

    st.subheader("Promedio de tasa de retención por año")
    ...
    grafico_retencion = px.line(
        ...
        y=COL_RETENCION,
        markers=True,
        title="Tasa de retención promedio por año"
    )
    st.plotly_chart(grafico_retencion, use_container_width=True)

#Pestaña 2: Facultades
with pestana2:
    ...
    grafico_dispersion = px.scatter(
        ...
        title="Satisfacción estudiantil vs Tasa de retención"
    )
    ...
    st.write(f"📈 Correlación entre satisfacción y retención: **{correlacion:.3f}**")

#Pestaña 3: Tabla y descarga
with pestana3:
    ...
    enlace_descarga = f'<a href="data:file/csv;base64,{b64}" download="datos_filtrados.csv">📥 Descargar datos filtrados</a>'
    st.markdown(enlace_descarga, unsafe_allow_html=True)

st.markdown("---")
st.caption("Panel desarrollado en Streamlit — Datos institucionales de admisión, retención y satisfacción estudiantil.")